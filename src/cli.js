...
program
  ...
  .option("--profile <name>", `built-in profile: ${Object.keys(builtInProfiles).join(", ")}`)
  .option("--profile-file <file>", "JSON file with named scrape profiles")
  .option("--crawl", "follow same-domain links starting from the provided URL", false)
  .option("--max-pages <number>", "maximum pages to crawl when --crawl is enabled", value => Number(value))
